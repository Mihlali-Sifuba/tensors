"""NumPy implementation of in-place indexed writing."""
...
def assign_indices(
    ...
    *,
    ...
    """Scatter the values into the addressed positions of the array itself.
    ...
    if not len(indices):
        return True
    positions = numpy.asarray(indices, dtype=numpy.intp)
    if not isinstance(values, Storage):
        buffer[positions] = values
        return True
    source = values.buffer
    if source_indices is not None:
        source = source[numpy.asarray(source_indices, dtype=numpy.intp)]
