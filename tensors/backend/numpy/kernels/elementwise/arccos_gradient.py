"""NumPy implementation of the arccosine VJP."""
...
def arccos_gradient(grad: Tensor, value: Tensor) -> Storage | None:
    ...
    if bool(numpy.any((values == -1.0) | (values == 1.0))):
        raise ValueError(f"{'arccos'} derivative is undefined at -1 and 1")
    if bool(numpy.any((values < -1.0) | (values > 1.0))):
        return None
