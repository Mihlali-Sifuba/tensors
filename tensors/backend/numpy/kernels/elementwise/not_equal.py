"""NumPy implementation of not_equal."""
...
def not_equal(
    ...
    """Evaluate the broadcasting not_equal comparison on native arrays."""
    expected_shape = tuple(output_shape)
    if any(
        tuple(values.shape) != expected_shape for values in (left_values, right_values)
    ):
        raise RuntimeError(
            "not_equal kernel received operands not prepared for output_shape"
        )
    result = numpy.not_equal(left_values, right_values).astype(numpy.uint8, copy=False)
