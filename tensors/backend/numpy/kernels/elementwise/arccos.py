"""NumPy implementation of arccosine."""
...
def arccos(value: Tensor, *, dtype: DataType) -> Storage | None:
    ...
    if bool(numpy.any((values < -1.0) | (values > 1.0))):
        raise ValueError(f"{'arccos'} is only defined for values between -1 and 1")
    functions = {
        "abs": numpy.abs,
        "sqrt": numpy.sqrt,
        "exp": numpy.exp,
    ...
    with _errstate(divide="ignore", over="ignore", under="ignore", invalid="ignore"):
        result = functions["arccos"](values)
