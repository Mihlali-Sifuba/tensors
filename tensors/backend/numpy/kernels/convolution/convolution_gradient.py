...
from tensors.backend.numpy.kernels.convolution.backward import _convolution_scatter_add
...
def convolution_gradient(
    ...
    *,
    ...
    kernel_result = numpy.zeros(kernel_values.shape, dtype=upstream.dtype)
    ...
    with _errstate(over="ignore", under="ignore", invalid="ignore"):
        for batch_slice, output_start, output_extent in _convolution_tiles(
            ...
            if need_kernel:
                kernel_result += (
                    numpy.matmul(upstream_matrix, column_matrix.transpose(0, 1, 3, 2))
                    .sum(axis=0)
