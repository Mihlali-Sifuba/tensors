"""CUDA-native softmax vector-Jacobian product."""
...
def softmax_gradient(
    ...
    *,
    ...
    with _errstate(over="ignore", under="ignore", invalid="ignore"):
        ...
        zero = cupy.zeros_like(weighted[..., :1])
        prefix = cupy.concatenate(
            (zero, cupy.cumsum(weighted, axis=-1)[..., :-1]), axis=-1
        )
        reversed_suffix = cupy.cumsum(weighted[..., ::-1], axis=-1)[..., ::-1]
        suffix = cupy.concatenate((reversed_suffix[..., 1:], zero), axis=-1)
        centered = normalized * moved_complements - prefix - suffix
        centered = cupy.where(scalable, centered * safe_scale, centered)
        rounded = _widen(_narrow(centered, cupy.dtype(dtype.name)))
        result = moved_probabilities * rounded
        spread = cupy.max(moved_grad, axis=-1, keepdims=True) - cupy.min(
