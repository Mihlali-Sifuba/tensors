"""CUDA implementation of less_equal."""
...
def less_equal(
    ...
    """Evaluate the broadcasting less_equal comparison on device arrays."""
    expected_shape = tuple(output_shape)
    if any(
        tuple(values.shape) != expected_shape for values in (left_values, right_values)
    ):
        raise RuntimeError(
            "less_equal kernel received operands not prepared for output_shape"
        )
    ...
    result = cupy.less_equal(left_values, right_values).astype(cupy.uint8, copy=False)
