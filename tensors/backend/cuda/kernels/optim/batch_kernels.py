"""Fused CUDA optimizer kernels, compiled on first use.

Each kernel applies one optimizer's update to a whole batch of parameters and
raises a shared device flag when any operand or result is not finite, so the
caller can decline the batch and fall back to individual updates.
"""
