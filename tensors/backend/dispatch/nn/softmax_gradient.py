"""Strict selected-backend dispatch for the softmax VJP."""
...
def execute_softmax_gradient(grad: Tensor, value: Tensor, axis: int) -> Storage:
    """Run the softmax VJP on the selected backend without fallback."""
    ...
    result = backend.softmax_gradient(
        grad_values,
        values,
        grad.shape,
        value.shape,
    ...
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute softmax_gradient at dtype "
            f"{grad.dtype.name} conformingly. The VJP runs on the selected "
