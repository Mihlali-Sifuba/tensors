"""Dispatch for the ReLU VJP: the selection alone decides where it runs.

`docs/relu-semantics.md` section 6 states the contract this dispatcher must
...
Nothing here inspects operand values. The ReLU VJP has no domain error — the
kink uses a chosen subgradient rather than raising — so no reduction and no
...
def execute_relu_gradient(
    ...
    *,
    ...
    """Run the ReLU VJP on the selected backend, or say it cannot run there."""
    ...
    result = backend.relu_gradient(
        lowered_grad, lowered_value, dtype=dtype, output_shape=output_shape
    )
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute relu_gradient at dtype "
