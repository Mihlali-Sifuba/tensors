"""Strict dispatch for less_equal."""
...
def execute_less_equal(
    left: Tensor,
    right: Tensor,
    *,
    output_shape: tuple[int, ...],
) -> Storage:
    """Run less_equal on the selected backend without fallback."""
    ...
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute less_equal conformingly. "
            f"less_equal runs on the selected backend; select another backend "
