"""Dispatch for sigmoid: the selection alone decides where it runs.

`docs/backends.md`, *Execution requirements*, makes the backend selection an
execution requirement rather than a preference. Sigmoid executes on the
...
No workload-size policy applies: a one-element sigmoid runs where a
million-element sigmoid runs.
...
Nothing here inspects operand values. Sigmoid has no domain error — every
...
promoted to ``float64`` by :class:`~tensors.operations.activations.sigmoid.Sigmoid`,
...
def execute_sigmoid(
    ...
    *,
    ...
    """Run sigmoid on the selected backend, or report that it cannot run there."""
    ...
    if selected == "python":
        ...
        # shape. Sigmoid is unary, so there is no scalar or broadcasting case.
    ...
    result = backend.sigmoid(lowered, dtype=dtype, output_shape=output_shape)
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute sigmoid at dtype "
            f"{dtype.name} conformingly. Sigmoid runs on the selected backend; "
