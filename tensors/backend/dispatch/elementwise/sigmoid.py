...
def execute_sigmoid(value: Tensor, *, dtype: DataType) -> Storage:
    """Run an accelerated unary transform, or the Python reference."""
    from tensors.backend.python.kernels.elementwise.sigmoid import sigmoid as reference

    if not _array_work_is_large_enough(value.size, _NUMPY_ELEMENTWISE_MIN_SIZE):
        return reference(value, dtype=dtype)
    unary = _backend_kernel("sigmoid")
