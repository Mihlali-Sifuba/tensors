...
def execute_sin(
    ...
    *,
    ...
    """Run sin on the selected backend without cross-backend fallback."""
    ...
    result = backend.sin(lowered, dtype=dtype, output_shape=output_shape)
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute sin at dtype "
            f"{dtype.name} conformingly. sin runs on the selected backend; "
