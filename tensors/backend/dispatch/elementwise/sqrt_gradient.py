"""Dispatch for the square-root VJP: the selection decides where it runs.

`docs/sqrt-semantics.md` section 6 states the contract this dispatcher must
...
Nothing here inspects operand values. The zero domain error is enforced in
the kernels, where the native operand already is, and a negative primal is a
value with a NaN result rather than a condition to detect.
"""
...
def execute_sqrt_gradient(
    ...
    *,
    ...
    """Run the sqrt VJP on the selected backend, or say it cannot run there."""
    ...
    result = backend.sqrt_gradient(
        lowered_grad, lowered_value, dtype=dtype, output_shape=output_shape
    )
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute sqrt_gradient at dtype "
