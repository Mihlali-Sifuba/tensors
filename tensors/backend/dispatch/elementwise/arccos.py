...
def execute_arccos(
    ...
    *,
    ...
    """Run arccos on the selected backend without cross-backend fallback."""
    ...
    result = backend.arccos(lowered, dtype=dtype, output_shape=output_shape)
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute arccos at dtype "
            f"{dtype.name} conformingly. arccos runs on the selected backend; "
