"""Strict selected-backend dispatch for argmax."""
...
def execute_argmax(
    ...
    """Run argmax on the selected backend without fallback."""
    ...
    result = backend.argmax(
        values, value.shape, axis, keepdims=keepdims, output_shape=output_shape
    )
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute argmax conformingly. "
