"""Strict selected-backend dispatch for reduce_std_gradient."""
...
def execute_reduce_std_gradient(
    grad: Tensor, value: Tensor, axes: tuple[int, ...], *, keepdims: bool
) -> Storage:
    """Run reduce_std_gradient on the selected backend without fallback."""
    ...
    result = backend.reduce_std_gradient(
        grad_values,
        values,
        value.shape,
        axes,
    ...
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute reduce_std_gradient conformingly. "
