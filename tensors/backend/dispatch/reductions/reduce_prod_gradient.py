"""Strict selected-backend dispatch for reduce_prod_gradient."""
...
def execute_reduce_prod_gradient(
    grad: Tensor, value: Tensor, axes: tuple[int, ...], *, keepdims: bool
) -> Storage:
    """Run reduce_prod_gradient on the selected backend without fallback."""
    ...
    result = backend.reduce_prod_gradient(
        grad_values,
        values,
        value.shape,
        axes,
    ...
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute reduce_prod_gradient conformingly. "
