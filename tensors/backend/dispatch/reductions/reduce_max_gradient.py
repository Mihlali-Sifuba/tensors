"""Strict selected-backend dispatch for reduce_max_gradient."""
...
def execute_reduce_max_gradient(
    grad: Tensor, value: Tensor, axes: tuple[int, ...], *, keepdims: bool
) -> Storage:
    """Run reduce_max_gradient on the selected backend without fallback."""
    ...
    result = backend.reduce_max_gradient(
        grad_values,
        values,
        value.shape,
        axes,
    ...
    if result is None:
        raise BackendOperationUnsupportedError(
            f"The {selected} backend cannot execute reduce_max_gradient conformingly. "
