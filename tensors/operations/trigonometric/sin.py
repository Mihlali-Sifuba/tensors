"""Elementwise sin and its differentiation rule."""
...
class Sin(Operation):
    """Elementwise sin with a reverse-mode gradient rule."""
    ...
    def forward(self, value: Tensor) -> Tensor:
        ...
        return Tensor._from_owned_storage(
            backend_dispatch.execute_sin(value, dtype=dtype, output_shape=output_shape),
            dtype=dtype,
            shape=output_shape,
        )
    ...
    def backward(
        ...
        """Apply the first-order VJP ``G * cos(x)``."""
        ...
        return [
            Tensor._from_owned_storage(
                backend_dispatch.execute_sin_gradient(
...
def sin(value: TensorLike | VariableNode) -> TensorResult | VariableNode:
    """Return the elementwise sin of a graph value or Tensor."""
