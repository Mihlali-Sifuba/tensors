...
class Minimum(_ElementwiseExtremum):
    ...
    def backward(
        ...
        if grad.shape != output_shape:
            raise ValueError(
                f"Gradient shape {grad.shape} does not match minimum shape "
                f"{output_shape}"
            )
        dtype = result_dtype(left.dtype, right)
        if grad.dtype is not dtype:
            raise ValueError(
                f"Gradient dtype {grad.dtype.name} does not match minimum dtype "
                f"{dtype.name}"
            )
        if is_graph_operand(grad):
            ...
            for needed, operand, select_left in (
                ...
                contribution = apply_operation(
                    MinimumVJP(select_left=select_left), (grad, left, right)
                )
...
class MinimumVJP(Operation):
    ...
    def forward(self, grad: Tensor, left: Tensor, right: Tensor) -> Tensor:
        ...
        left_storage, right_storage = execute_minimum_gradient(
            grad,
            left,
            right,
            dtype=grad.dtype,
        ...
        if storage is None:
            raise RuntimeError("minimum VJP did not return its requested branch")
    ...
    def backward(
        ...
        if needs_input_grad[0]:
            operation = MinimumVJP(select_left=self.select_left)
            grad_partial = (
