...
from tensors.backend import execute_logsumexp, execute_logsumexp_gradient
...
class LogSumExpGradient(Operation):
    ...
    def forward(self, grad: Tensor, value: Tensor) -> Tensor:
        ...
        return LogSumExp(axis=axis, keepdims=keepdims).backward(
            grad, value, needs_input_grad=UNARY_DEMAND
        )[0]
    ...
    def backward(
        ...
        for output_index, group in enumerate(groups):
            ...
            if any((math.isnan(item) for item in group_values)):
                ...
                weights = [
                    1.0 / count if item == math.inf else 0.0 for item in group_values
                ]
