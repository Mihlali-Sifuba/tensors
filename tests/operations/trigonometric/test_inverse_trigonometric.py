...
class InverseTrigonometricTests(unittest.TestCase):
    ...
    def test_functions_return_elementwise_values_and_preserve_shape(self):
        ...
        for actual, expected in zip(
            inverse_sine.tolist(), map(math.asin, values.tolist())
        ):
            self.assertAlmostEqual(actual, expected)
        for actual, expected in zip(
            inverse_cosine.tolist(), map(math.acos, values.tolist())
        ):
            self.assertAlmostEqual(actual, expected)
        for actual, expected in zip(
            inverse_tangent.tolist(), map(math.atan, values.tolist())
        ):
            self.assertAlmostEqual(actual, expected)
    ...
    def test_arcsin_and_arccos_values_exist_at_domain_endpoints(self):
        self.assertEqual(
            ts.arcsin([-1.0, 1.0]).tolist(), [-math.pi / 2.0, math.pi / 2.0]
        )
    ...
    def test_composite_expression_passes_gradcheck(self):
        ...
        self.assertTrue(
            ts.gradcheck(
                lambda value: (ts.arcsin(value) + ts.arccos(value) + ts.arctan(value)),
                values,
            )
        )
