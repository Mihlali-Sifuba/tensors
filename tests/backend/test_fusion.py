...
class CudaFusionTests(unittest.TestCase):
    ...
    def test_graph_replay_fuses_post_matmul_bias_and_activation(self):
        with ts.use_backend("cuda"):
            value = ts.Variable(ts.full((64, 64), 1.0), requires_grad=False)
            weight = ts.Variable(ts.full((64, 64), 1.0), requires_grad=False)
    ...
    def test_extended_unary_and_power_chains_fuse_in_both_directions(self):
        ...
        with ts.use_backend("python"):
            ...
            reference_gradient = ts.grad(
                reference_output, reference_input, ts.ones((4_096,))
            )
        with ts.use_backend("cuda"):
    ...
    def test_fused_extreme_power_and_division_vjps_retain_range(self):
        with ts.use_backend("cuda"):
            base = ts.Variable(ts.full((4_096,), 1e-200))
            powered = base**3.0
            ...
            with patch.object(
                ...
                backend_state._clear_backend_kernel_cache()
                power_computation.backward(ts.full((4_096,), 1e308))
            numerator = ts.Variable(ts.full((4_096,), 1e308))
            denominator = ts.Variable(ts.full((4_096,), 1e308))
            quotient = numerator / denominator
    ...
    def test_integer_graphs_execute_on_the_device_unfused(self):
        ...
        for dtype in integer_dtypes:
            with self.subTest(dtype=dtype.name), ts.use_backend("cuda"):
                value = ts.Variable(
                    ts.full((4_096,), 2, dtype=dtype), requires_grad=False
                )
                output = (value + 1) * 2
                computation = ts.graph.Computation(output)
                with patch.object(
