...
class CudaExtremeGradientTests(unittest.TestCase):
    ...
    def test_extreme_vjps_remain_device_resident(self):
        with ts.use_backend("cuda"):
            numerator = ts.Variable(ts.full((64,), 1e308))
            denominator = ts.Variable(ts.full((64,), 1e308))
            division_gradient = ts.grad(
                numerator / denominator, denominator, ts.full((64,), 1e308)
            )
            base = ts.Variable(ts.full((64,), 1e-308))
            base_gradient = ts.grad(base**2.0, base, ts.full((64,), 1e308))
            exponent_base = ts.Variable(ts.full((64,), 1e-200))
            exponent = ts.Variable(ts.full((64,), 3.0))
            exponent_gradient = ts.grad(
                exponent_base**exponent, exponent, ts.full((64,), 1e308)
            )
            broadcast_value = ts.Variable([0.0])
            factor = ts.Variable([1e308, -1e308], requires_grad=False)
    ...
    def test_derivative_helper_operations_dispatch_to_numpy(self):
        with (
            ...
            with ts.use_backend("numpy"):
                ...
                ts.grad(ts.sum(3.0 / denominator), denominator, create_graph=True)
