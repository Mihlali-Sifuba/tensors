...
class NumPyLayoutTests(NumPyParityTestCase):
    ...
    def test_creation_layout_and_optimizers_dispatch_to_numpy(self):
        with (
            patch.object(numpy_backend, "full", wraps=numpy_backend.full) as full,
            patch.object(numpy_backend, "eye", wraps=numpy_backend.eye) as eye,
            patch.object(numpy_backend, "arange", wraps=numpy_backend.arange) as arange,
            patch.object(
                numpy_backend, "linspace", wraps=numpy_backend.linspace
            ) as linspace,
            patch.object(
                numpy_backend, "transpose", wraps=numpy_backend.transpose
            ) as transpose,
            patch.object(numpy_backend, "concat", wraps=numpy_backend.concat) as concat,
            patch.object(numpy_backend, "stack", wraps=numpy_backend.stack) as stack,
            patch.object(numpy_backend, "outer", wraps=numpy_backend.outer) as outer,
            patch.object(
                numpy_backend, "outer_gradient", wraps=numpy_backend.outer_gradient
            ) as outer_gradient,
            patch.object(
                numpy_backend, "sgd_update", wraps=numpy_backend.sgd_update
            ) as sgd_update,
            patch.object(
                numpy_backend, "adam_update", wraps=numpy_backend.adam_update
            ) as adam_update,
            patch.object(
                numpy_backend, "rmsprop_update", wraps=numpy_backend.rmsprop_update
            ) as rmsprop_update,
        ):
            with ts.use_backend("numpy"):
                ...
                ts.grad(product, (left, right), grad_outputs=ts.full((8, 8), 1.0))
    ...
    def test_slice_scatter_dispatches_to_numpy(self):
        with patch.object(
            numpy_backend, "slice_scatter", wraps=numpy_backend.slice_scatter
        ) as slice_scatter:
            with ts.use_backend("numpy"):
    ...
    def test_cast_dispatches_to_numpy(self):
        ...
        with patch.object(
            numpy_backend, "cast_tensor", wraps=numpy_backend.cast_tensor
        ) as cast_tensor:
    ...
    def test_slice_and_cast_match_python_backend(self):
        ...
        self.assertOperationParity(lambda: value.astype(ts.float32))
