"""Activation functions, measured at float64 only.

These exist to shape a layer's output, so the question is what
one costs in a network rather than how it varies with dtype; the
dtype question is answered by the elementary functions, which
share the same per-element shape."""
...
OPERATIONS: tuple[str, ...] = ("relu", "sigmoid", "softplus")
    ...
    for operation in OPERATIONS:
        for dtype_name in ("float64",):
            for size in selected_sizes(SIZES):
                ...
                result.append(
                    Group(
                        name=f"activations/{operation}/{dtype_name}/{size}",
                        factory=factory,
                        suite="activations",
                    )
                )
