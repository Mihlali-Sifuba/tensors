...
Separated from the elementary functions because a backend can
implement them very differently: a library call per element on
one backend, a fused device kernel on another."""
    ...
    for operation in OPERATIONS:
        for dtype_name in FLOAT_DTYPES:
            for size in selected_sizes(SIZES):
                ...
                result.append(
                    Group(
                        name=f"trigonometric/{operation}/{dtype_name}/{size}",
                        factory=factory,
                        suite="trigonometric",
                    )
                )
