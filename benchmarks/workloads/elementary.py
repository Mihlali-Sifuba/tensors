"""Magnitude, sign, roots, exponential, and logarithm.

The functions whose cost is one pass over the buffer with a real
arithmetic operation per element, which is what separates them
from a comparison and from a reduction."""
...
OPERATIONS: tuple[str, ...] = ("exp", "log", "sqrt", "abs", "sign")
    ...
    for operation in OPERATIONS:
        for dtype_name in FLOAT_DTYPES:
            for size in selected_sizes(SIZES):
                ...
                result.append(
                    Group(
                        name=f"elementary/{operation}/{dtype_name}/{size}",
                        factory=factory,
                        suite="elementary",
                    )
                )
