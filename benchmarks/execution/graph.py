...
from ..scenarios.models.linear import Linear, Nested
...
def _wide_nodes(count: int) -> tuple[VariableNode, tuple[VariableNode, ...]]:
    ...
    state = get_graph_state()
    leaf = state.add_variable_node()
    branches = [state.record_operation(Mul(), (leaf, leaf)) for _ in range(count)]
    current = branches[0]
...
def _structural_cases(backend: str, topology: str, count: int) -> list[Case]:
...
def _execution_cases(backend: str, topology: str, count: int, width: int) -> list[Case]:
    ...
    cases.append(
        Case(
            ...
            layer="public",
            validate=run_tensor,
            description=(f"{count} plain Tensor operations, with no graph recorded"),
            **common,
        )
    )
...
def _model_cases(backend: str, batch: int, features: int) -> list[Case]:
    ...
    cases: list[Case] = []
    inputs = tensor((batch, features), dtype_name="float64", kind="constant", value=0.5)

    model = Linear(features, features)
    model(inputs)
    cases.append(
        Case(
    ...
    nested = Nested(features, features)
    nested(inputs)
    cases.append(
        Case(
    ...
    construct_common["tags"] = {
        **common["tags"],
        "curve": f"graph-model-construct|{features}",
    }
    cases.append(
        Case(
    ...
    traced_model = ts.Graph(lambda values: ts.relu(values * 2.0) + 1.0)
    traced_model(inputs)
    trace_common = dict(common)
    trace_common["tags"] = {
        **common["tags"],
    ...
    compiled_model = ts.Graph(lambda values: ts.relu(values * 2.0) + 1.0)
        ...
        for count in (1, 10, 100, 1_000):
            for width in WIDTHS:
                ...
                def execution(
                    ...
                    return _execution_cases(backend, topology, count, width)
    ...
    for batch, features in ((1, 16), (32, 128), (128, 512)):
        ...
        def model(
            ...
            if backend == "python" and batch * features > 20_000:
                raise Unsupported("exceeds the Python backend model ceiling")
