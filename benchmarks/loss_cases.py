...
def _shape_cases(
    samples: int, classes: int, backends: frozenset[BenchmarkBackend] | None
) -> list[BenchmarkCase]:
    shape = (samples, classes)
    logits = ts.reshape(ts.linspace(-2.0, 2.0, samples * classes), shape)
    indices = ts.Tensor([sample % classes for sample in range(samples)], dtype=ts.int64)
    ...
    if backend in _ACCELERATED:
        ...
        def dense_kernel():
            return kernels.cross_entropy(
                logits, dense, 1, reduction="mean", dtype=ts.float64, output_shape=(1,)
            )
...
def cases() -> list[BenchmarkCase]:
    ...
    if ts.get_backend() != "python":
        benchmarks.extend(_shape_cases(1_024, 256, _ACCELERATED))
    return benchmarks
