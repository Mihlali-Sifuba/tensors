"""Derived tables: layer ladders, ratios, crossovers, and rankings.

Nothing here measures anything. It reads the records a run produced and
answers the comparison questions those records were designed for. A case
joins a comparison by carrying a ``ladder`` tag: every layer measuring the
same computation on the same dtype and size shares one ladder key, so the
ratios between layers can be resolved without naming cases pairwise.
"""
...
def build_analysis(records: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    ladder_entries = ladders(records)
    return {
        "ladders": ladder_entries,
        "scaling_curves": scaling_curves(records),
        "crossovers": crossovers(records),
        "dtype_comparison": dtype_comparison(records),
        "layout_comparison": layout_comparison(records),
        "forward_versus_backward": forward_backward(records),
        "trace_versus_replay": trace_replay(records),
        "reduction_guard": guard_comparison(records),
        "training_breakdown": training_breakdown(records),
        "cuda_synchronization": cuda_synchronization(records),
        "memory": memory_findings(records),
        "noisy_cases": noisy_cases(records),
        "classifications": classification_summary(records),
        "bottlenecks": bottlenecks(records, ladder_entries),
    }
