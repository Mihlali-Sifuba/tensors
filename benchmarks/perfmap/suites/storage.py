...
from tensors.backend.storage import convert_storage
...
#: The storage representation each backend produces natively.
_NATIVE_KIND = {"python": "python", "numpy": "numpy", "cuda": "cuda"}


def _construction_cases(
    backend: str, size: int, dtype_name: str
) -> list[Case]:
    ...
    if backend in ACCELERATED:
        ...
        # The public constructor accepts scalars, lists, ``array.array``,
        # ``Tensor`` and ``Storage`` — not a provider array. Passing one is
        # therefore not a slow path but an unsupported one, and it is
        ...
        def validate_unsupported() -> None:
            try:
                ...
                raise Unsupported(
                    "Tensor() does not accept a provider array: "
                    f"{error}. The supported route from provider values is "
                    "to wrap them in the matching Storage class, measured "
                    "as storage.from_provider_storage"
                ) from error
        ...
        cases.append(Case(
            ...
            description=(
                f"construct a Tensor directly from a {provider.__name__} "
                "array"
            ),
        ...
        # The route that does work: wrap the provider buffer in Storage and
        # hand that to the constructor. Public construction copies, so the
        # internal owned-storage path is measured beside it.
        ...
        cases.append(Case(
            ...
            description=(
                "construct a Tensor from provider-backed Storage, which "
                "public construction copies for independent ownership"
            ),
            backends=ACCELERATED,
            memory=True,
        ...
        cases.append(Case(
            ...
            description=(
                "the internal path every accelerated operation ends with: "
                "adopt a provider result without copying it"
            ),
            backends=ACCELERATED,
            memory=True,
        ...
        if backend == "cuda":
            ...
            cases.append(Case(
                ...
                run=lambda: _native_storage(
                    "cuda", host_native, dtype
                ),
                layer="storage",
                validate=lambda: _native_storage(
                    "cuda", host_native, dtype
                ),
                description=(
                    "move host NumPy values into device storage, which is "
                    "the transfer a device Tensor needs"
                ),
...
def _conversion_cases(
    ...
    # -- first conversion against repeated lookup ------------------------
    #
    # The first request for a representation converts and caches it; later
    # requests find it. A fresh Tensor per sample is what makes the first
    # request measurable, so those cases reset their own state.
    for target_kind in ("python", "numpy", "cuda"):
        ...
        def reset(
            holder: dict[str, ts.Tensor] = holder,
        ) -> None:
            holder["value"] = tensor(
                shape, dtype_name=dtype_name, kind="ramp"
            )
        ...
        cases.append(Case(
            ...
            single_shot=True,
            description=(
                f"first request for the {target_kind} representation of a "
                f"{native_kind}-backed Tensor, which converts and caches it"
            ),
            memory=True,
            **first_common,
        ))
        ...
        cases.append(Case(
            ...
            layer="storage",
            validate=lambda warm=warm, target_kind=target_kind: (
                warm._storage_for(target_kind)
            ),
            description=(
                f"repeated request for an already-cached {target_kind} "
                "representation"
            ),
            **common(f"cached_lookup_{target_kind}"),
        ))

    # -- direct conversion between representations -----------------------
    ...
    for target_kind in ("python", "numpy", "cuda"):
        ...
        cases.append(Case(
            name=(
                f"storage.convert/{native_kind}-to-{target_kind}/"
                f"{dtype_name}/{size}"
            ),
            ...
            description=(
                f"convert {native_kind} storage to {target_kind} storage"
            ),
            memory=True,
            **common(f"convert_{native_kind}_to_{target_kind}"),
        ))

    # -- copy, materialization, dtype conversion -------------------------
    cases.append(Case(
    ...
    cases.append(Case(
        ...
        description=(
            "materialize every logical value as a Python list, which "
            "requires the host representation"
        ),
        memory=True,
    ...
    cases.append(Case(
        ...
        single_shot=True,
        description=(
            "an in-place write, which installs host storage as "
            "authoritative and discards the other cached representations"
        ),
        **common("invalidate"),
    ))

    # -- scalar extraction: the host/device barrier ----------------------
    ...
    # Equality compares every value through the host list, so it is a
    ...
    cases.append(Case(
        ...
        description=(
            "Tensor equality, which compares host lists and therefore "
            "materializes both operands"
        ),
        **common("equality"),
    ))
    return cases
        ...
        for size in (1, 100, 10_000, 1_000_000):
            def construction(
                ...
                if backend == "python" and size > 100_000:
                    raise Unsupported(
                        "exceeds the Python backend construction ceiling"
                    )
            ...
            def conversion(
                ...
                if size > 100_000 and backend == "python":
                    raise Unsupported(
                        "exceeds the Python backend conversion ceiling"
                    )
