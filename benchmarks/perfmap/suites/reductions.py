...
from tensors.backend import execute_reduction, execute_reduction_gradient
from tensors.backend.kernels.core import _numpy, _view
from tensors.backend.kernels.reductions.stability import (
    _scaled_sum,
    _summation_guard,
)
...
#: Public reduction entry points by internal operation name.
_PUBLIC = {
    "sum": ts.sum,
    "mean": ts.mean,
    "min": ts.min,
...
#: The provider primitive each reduction is ultimately built from.
_PROVIDER_NAME = {
    "sum": "sum",
    "mean": "mean",
    "min": "min",
...
#: Reductions with a differentiable VJP kernel.
_DIFFERENTIABLE = ("sum", "mean", "min", "max", "prod", "variance", "std")


def _provider_call(provider: Any, name: str, array: Any, axes: Any,
                   keepdims: bool) -> Any:
    """Invoke the provider primitive matching a reduction."""
    if name == "linalg.norm":
        # ``linalg.norm`` takes no keepdims on every provider version, so a
        # full-array Frobenius norm is the comparable primitive.
...
def _reduction_cases(
    ...
    axis_label = "all" if axis is None else (
            "-".join(str(item) for item in axis)
            if isinstance(axis, tuple)
            else str(axis)
        )
    suffix = (
        f"{dtype_name}/{'x'.join(str(item) for item in shape)}"
        f"/axis-{axis_label}/keepdims-{int(keepdims)}/{data_kind}"
    )
    ...
    # Axes as the kernel expects them: an explicit tuple over every axis.
    if axis is None:
    ...
    if backend in ACCELERATED:
        ...
        raw = provider_array(
            provider, shape, dtype_name=dtype_name, kind=data_kind
        )
        ...
        def run_provider() -> Any:
            return _provider_call(
                ...
                None if axis is None else axes, keepdims,
            )
        ...
        # The guard components, isolated. These are the two extra passes a
        # guarded summation performs over the same values, plus the
        # host-visible decision that consumes them.
        if operation in ("sum", "mean"):
            array_module = _numpy()
            working = _view(value, array_module).astype(
            ...
            cases.append(Case(
                ...
                run=lambda: _summation_guard(
                    working, array_module, axes=axes, keepdims=True
                ),
                layer="kernel",
                ...
                description=(
                    "the reduction stability predicate alone, without the "
                    "host-visible bool() that consumes it"
                ),
                backends=ACCELERATED,
                **guard_common,
            ))
            cases.append(Case(
                ...
                run=lambda: bool(_summation_guard(
                    working, array_module, axes=axes, keepdims=True
                )),
                layer="kernel",
                validate=lambda: bool(_summation_guard(
                    working, array_module, axes=axes, keepdims=True
                )),
                description=(
                    "the stability predicate plus the bool() conversion, "
                    "which is a host/device barrier on CUDA"
                ),
                backends=ACCELERATED,
                **guard_common,
            ))
            cases.append(Case(
                ...
                run=lambda: _scaled_sum(working, axes, array_module),
                layer="kernel",
                validate=lambda: _scaled_sum(working, axes, array_module),
        ...
        def run_kernel() -> Any:
            return kernels.reduction(
                ...
                keepdims=keepdims, dtype=dtype, output_shape=output_shape,
            )

        def validate_kernel() -> None:
            if run_kernel() is None:
                raise Unsupported(
                    "the guarded reduction kernel declines this "
                    "configuration and defers to the Python reference "
                    "implementation"
                )
        ...
        cases.append(Case(
            ...
            run=lambda: execute_reduction(
                operation, value, axes,
                keepdims=keepdims, dtype=dtype, output_shape=output_shape,
            ),
            layer="dispatch",
            validate=lambda: execute_reduction(
                operation, value, axes,
                keepdims=keepdims, dtype=dtype, output_shape=output_shape,
            ),
    ...
    # The VJP kernel over the same configuration, so backward can be
    # compared against the forward reduction it differentiates.
    if backend in ACCELERATED and operation in _DIFFERENTIABLE:
        ...
        def run_gradient() -> Any:
            return execute_reduction_gradient(
                operation, gradient, value, axes, keepdims=keepdims
            )

        def validate_gradient() -> None:
            if run_gradient() is None:
                raise Unsupported(
                    "the reduction VJP kernel declines this configuration "
                    "and defers to the Python reference implementation"
                )
...
def _normalization_cases(
    ...
    if backend in ACCELERATED and operation in ("softmax", "log_softmax"):
        ...
        def run_kernel() -> Any:
            return kernels.normalization(
                operation, value, -1, dtype=dtype_of(dtype_name)
            )
...
def _arg_extremum_cases(
    ...
    if backend in ACCELERATED:
        ...
        def run_kernel() -> Any:
            return kernels.arg_extremum(
                operation, value, None, keepdims=False, output_shape=(1,)
            )

        def validate_kernel() -> None:
            if run_kernel() is None:
                raise Unsupported(
                    "the arg-extremum kernel declines this configuration"
                )
    ...
    *,
    ...
    def factory(backend: str) -> Sequence[Case]:
        if elements > limits[backend]:
            raise Unsupported(
                f"{elements} elements exceeds the {backend} reduction "
                f"ceiling of {limits[backend]}; guarded reductions build "
                "several full-size temporaries"
            )
...
def groups() -> list[Group]:
    ...
    # -- the full size curve, full reduction, both data patterns ---------
        ...
        for dtype_name in FLOAT_DTYPES:
            for size in (1, 100, 10_000, 1_000_000, 4_000_000):
                for data_kind in ("ramp", "mixed"):
                    _register(
                        result,
                        f"reductions/{operation}/{dtype_name}/{size}/"
                        f"{data_kind}",
                        lambda backend, operation=operation,
                        dtype_name=dtype_name, size=size,
                        data_kind=data_kind: _reduction_cases(
                            backend, operation, dtype_name, (size,),
                            None, False, data_kind,
                        ),
                        elements=size,
                    )

    # -- axis variation on a matrix, with and without keepdims -----------
    for operation in ("sum", "mean", "max", "std"):
        for shape in ((512, 512), (64, 16_384), (16_384, 64)):
            for axis in (None, 0, 1):
                for keepdims in (False, True):
                    if axis is None and keepdims:
                        # ``keepdims`` over every axis is covered by the
                        # curve above; this varies the axis instead.
                        continue
                    _register(
                        result,
                        f"reductions/axis/{operation}/"
                        f"{shape[0]}x{shape[1]}/{axis}/{int(keepdims)}",
                        lambda backend, operation=operation, shape=shape,
                        axis=axis, keepdims=keepdims: _reduction_cases(
                            backend, operation, "float64", shape,
                            axis, keepdims, "ramp",
                        ),
                        elements=shape[0] * shape[1],
                    )

    # -- multiple-axis reduction over a rank-3 tensor --------------------
    for operation in ("sum", "mean"):
        for axis in ((0, 1), (1, 2), (0, 2)):
            _register(
                result,
                f"reductions/multiaxis/{operation}/"
                f"{'-'.join(str(item) for item in axis)}",
                lambda backend, operation=operation,
                axis=axis: _reduction_cases(
                    backend, operation, "float64", (64, 64, 64),
                    axis, False, "ramp",
                ),
                elements=64 ** 3,
            )

    # -- normalizations and arg reductions --------------------------------
    for operation in ("softmax", "log_softmax", "logsumexp"):
        for dtype_name in FLOAT_DTYPES:
            for shape in ((32, 32), (256, 1_024), (1_024, 1_024)):
                _register(
                    result,
                    f"reductions/{operation}/{dtype_name}/"
                    f"{shape[0]}x{shape[1]}",
                    lambda backend, operation=operation,
                    dtype_name=dtype_name, shape=shape: _normalization_cases(
    ...
    for operation in ("argmax", "argmin"):
        for size in (100, 10_000, 1_000_000):
            _register(
                ...
                lambda backend, operation=operation,
                size=size: _arg_extremum_cases(backend, operation, (size,)),
                elements=size,
            )
