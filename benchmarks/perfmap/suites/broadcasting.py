...
#: Broadcast patterns worth separating, as (left shape, right shape).
...
def _broadcast_shape(
    ...
    return tuple(
        max(one, other) for one, other in zip(padded_left, padded_right)
    )
...
def _forward_cases(
    ...
    if backend in ACCELERATED:
        ...
        raw_right = provider_array(
            provider, right_shape, dtype_name="float64",
        ...
        def validate_kernel() -> None:
            if run_kernel() is None:
                raise Unsupported(
                    "the binary kernel declines this broadcast and defers "
    ...
    cases.append(Case(
        ...
        description=(
            "public broadcasting multiply, including shape agreement and "
...
def _gradient_cases(
    ...
    if backend not in ACCELERATED:
        raise Unsupported(
            "sum_to_shape and sum_products_to_shape are array kernels; the "
            "Python backend reduces broadcast gradients in its reference "
            "reduction path instead"
        )
    output_shape = _broadcast_shape(left_shape, right_shape)
    if right_shape == output_shape:
        raise Unsupported(
            "this pattern does not broadcast the right operand, so no "
            "gradient reduction is performed for it"
        )
    elements = _elements(output_shape)
    common: dict[str, Any] = {
    ...
    # Which axes the provider would reduce to get back to the operand shape.
    rank = len(output_shape)
    padded = (1,) * (rank - len(right_shape)) + right_shape
    axes = tuple(
        index for index, (source, target) in enumerate(zip(output_shape, padded))
        if source != target
    )
    ...
    cases.append(Case(
        ...
        layer="provider",
        validate=lambda: provider.sum(
            raw_gradient, axis=axes
        ).reshape(right_shape),
    ...
    def validate_sum_to_shape() -> None:
        if run_sum_to_shape() is None:
            raise Unsupported(
                "sum_to_shape declined this configuration and deferred to "
    ...
    cases.append(Case(
        ...
        description=(
            "the guarded broadcast-gradient reduction used by reverse "
            "passes"
        ),
        **common,
    ))
    ...
    def validate_sum_products() -> None:
        if run_sum_products() is None:
            raise Unsupported(
                "sum_products_to_shape declined this configuration and "
...
def groups() -> list[Group]:
    ...
    for pattern, (left_shape, right_shape) in PATTERNS.items():
        ...
        def forward(
            ...
            if backend == "python" and elements > 100_000:
                raise Unsupported(
                    f"{elements} broadcast elements exceeds the Python "
                    "backend ceiling"
                )
    ...
    # A size curve for the single most common training pattern: a bias row
    # broadcast across a batch, and the gradient reduction back to it.
    for rows in (8, 64, 512, 4_096):
        ...
        def curve(
            ...
            return _forward_cases(
                backend, f"bias-row-{rows}x{columns}",
