...
def _fusion_cases(
    backend: str, depth: int, width: int, dtype_name: str
) -> list[Case]:
    ...
    cases: list[Case] = []
    value = tensor(
        (width,), dtype_name=dtype_name, kind="constant", value=0.5
    )
    ...
    if backend in ACCELERATED:
        ...
        raw = provider_array(
            provider, (width,), dtype_name=dtype_name,
    ...
    unfused_common["tags"] = {
        **common["tags"], "curve": f"fusion-unfused|{dtype_name}|d{depth}",
    }
    cases.append(Case(
...
def _operand_cases(
    backend: str, operands: int, width: int
) -> list[Case]:
    ...
    sources = [
        ts.Variable(
            tensor(
                (width,), dtype_name="float64", kind="constant",
        ...
        for depth in DEPTHS:
            for width in WIDTHS:
                def factory(
                    ...
                    return _fusion_cases(
                        backend, depth, width, dtype_name
                    )
