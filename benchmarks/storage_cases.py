...
def cases() -> list[BenchmarkCase]:
    ...
    for size in (100_000, 1_000_000):
        ...
        def validate_first_operation(
            run=first_public_operation, expected_size=size
        ) -> None:
            result = run()
        ...
        def validate_native_to_python(run=native_to_python, expected_size=size) -> None:
            storage = run()
            assert storage.size == expected_size
        ...
        if backend == "cuda":
            ...
            def validate_cuda_to_numpy(run=cuda_to_numpy, expected_size=size) -> None:
                storage = run()
                assert storage.size == expected_size
                assert float(storage.buffer[-1]) == 1.25
        ...
        def public_materialization(value=native_tensor, expected_size=size):
            fresh = ts.Tensor(
                value._storage.copy(), dtype=value.dtype, shape=value.shape
            )
            result = fresh.tolist()
        ...
        def persistent_state(expected_size=size, kind=native_kind):
        ...
        def reset_persistent(holder=persistent_holder, create=persistent_state) -> None:
            holder[0] = create()

        def mutation_invalidation(holder=persistent_holder, kind=native_kind):
            value = holder[0]
            value[0] = value[0] + 1.0
            return value._storage_for(kind)

        def validate_mutation_invalidation(
            run=mutation_invalidation, expected_size=size
        ) -> None:
            storage = run()
            assert storage.size == expected_size
            assert float(storage.buffer[-1]) == 1.25
        ...
        if backend == "cuda":
            ...
            def validate_host_device_roundtrip(
                run=host_device_roundtrip, expected_size=size
            ) -> None:
                storage = run()
                assert storage.size == expected_size
                assert float(storage.buffer[-1]) == 1.25
