...
def cases() -> list[BenchmarkCase]:
    ...
    arithmetic = load_backend(backend)
    ...
    for size in (100, 10_000, 100_000, 1_000_000, 10_000_000):
        ...
        def kernel_add(left=tensor_left, right=tensor_right, output_size=size):
            return arithmetic.add(
                left, right, dtype=ts.float64, output_shape=(output_size,)
            )
        ...
        def kernel_sum(values=tensor_left):
            return kernels.reduce_sum(
                values, (0,), keepdims=False, dtype=ts.float64, output_shape=(1,)
            )

        def validate_kernel_sum(run=kernel_sum, expected_size=size) -> None:
            storage = run()
            assert storage is not None
            assert math.isclose(_scalar(storage.buffer[0]), expected_size * 1.5)
    ...
    for size in (16, 64, 128, 256, 512, 1_024):
        ...
        def kernel_matmul(left=tensor_left, right=tensor_right, output_size=size):
            return kernels.matmul(
                left, right, dtype=ts.float64, output_shape=(output_size, output_size)
            )

        def validate_kernel_matmul(run=kernel_matmul, expected_value=expected) -> None:
            storage = run()
            assert storage is not None
